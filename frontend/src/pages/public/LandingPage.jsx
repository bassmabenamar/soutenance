...
const CONTENT = {
  nav: [
    { name: 'Accueil', path: '/' },
    { name: 'Catalogue', path: '/courses' },
    { name: 'Tarifs', path: '/buy-notebook' },
  ],
  hero: {
    ...
    <div className="min-h-screen bg-white font-sans text-slate-800">
      ...
      <nav className="h-20 flex items-center justify-between px-12 border-b border-slate-50 sticky top-0 bg-white/90 backdrop-blur-md z-50">
        <div className="flex items-center gap-8">
          <Link to="/" className="text-[#F97316] text-xl font-black uppercase tracking-tighter">
            CodeBook Academy
          </Link>
          <div className="hidden md:flex items-center gap-8 border-l pl-8 border-slate-100">
            {CONTENT.nav.map((item) => (
              <Link key={item.name} to={item.path} className="text-sm font-bold text-slate-400 hover:text-[#F97316] transition-colors">
                {item.name}
              </Link>
            ))}
          </div>
        ...
        <div className="flex items-center gap-4">
           <Link to="/login" className="text-sm font-black text-slate-400 px-4">Connexion</Link>
           <Link to="/buy-notebook" className="bg-[#F97316] text-white px-6 py-2.5 rounded-xl text-sm font-black shadow-lg shadow-orange-100">S'inscrire</Link>
        </div>
      </nav>
      ...
      <section className="px-12 py-24 bg-blue-50/30">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto">
          {CONTENT.techStack.map((tech, idx) => (
            <TechCard key={idx} label={tech.label} color={tech.color} />
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-16">
          <div className="col-span-2 space-y-6">
            ...
            <p className="text-slate-400 text-sm leading-relaxed max-w-xs">
              Former la prochaine génération d'ingénieurs logiciels.
            </p>
          </div>
...
const FeatureBlock = ({ className, icon, title, desc }) => (
  <div className={`p-10 rounded-[32px] border border-slate-50 shadow-sm space-y-6 transition-all hover:shadow-md ${className}`}>
    <div className="w-12 h-12 flex items-center justify-center bg-slate-50/10 rounded-2xl shadow-inner">
      {icon}
    </div>
...
const Step = ({ number, title, desc }) => (
  <div className="space-y-6 group">
    <div className="w-14 h-14 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-xl font-black text-slate-300 group-hover:bg-[#F97316] group-hover:text-white transition-all duration-500">
      {number}
    </div>
    <div>
