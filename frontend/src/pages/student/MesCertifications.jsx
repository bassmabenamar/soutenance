  ...
  useEffect(() => {
    const fetchCerts = async () => {
      try {
        ...
        // Fallback pour la démo si le backend n'est pas prêt
        setData(mockData); 
      } finally {
        setLoading(false);
      }
    ...
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans">
      ...
      >
        ...
        <motion.div variants={fadeInUp} className="relative bg-[#1e293b] rounded-[40px] p-12 overflow-hidden shadow-2xl">
            ...
            >
              ...
              <h2 className="text-2xl font-black text-white italic flex items-center justify-center gap-2">
                {data.current_badge} <Sparkles size={18} />
              </h2>
            </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <StatCard icon={<Award className="text-orange-500" />} label="Certificats" value={data.stats.total} />
          <StatCard icon={<BookOpen className="text-blue-500" />} label="Cours finis" value={data.stats.completed} />
          <StatCard icon={<TrendingUp className="text-emerald-500" />} label="Moyenne" value={`${data.stats.avg}%`} />
        </div>
